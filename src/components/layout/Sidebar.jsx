...
const Sidebar = () => {
  ...
  const NavItem = ({ to, icon, label, badge }) => (
    ...
    >
      <div className={`transition-transform duration-200 ${isActive(to) ? 'scale-110' : 'group-hover:scale-110'}`}>
        {icon}
      </div>
      <span className={`font-bold transition-all duration-300 whitespace-nowrap overflow-hidden ${isCollapsed ? 'lg:w-0 lg:opacity-0' : 'w-full opacity-100 text-sm'}`}>
        {label}
      </span>
      {!isCollapsed && badge && (
  ...
  return (
    <>
      ...
      {/* Mobile Overlay */}
      {isMobileOpen && (
        <div className="lg:hidden fixed inset-0 bg-secondary-900/40 backdrop-blur-sm z-40 transition-all duration-300" onClick={() => setIsMobileOpen(false)} />
      )}
      ...
      >
        ...
        <div className="p-4 border-t border-secondary-100 bg-secondary-50/30">
          <div className={`p-4 rounded-[1.5rem] bg-white border border-secondary-100 shadow-sm transition-all duration-300 ${isCollapsed ? 'px-2' : ''}`}>
             {!isCollapsed && (
               <div className="flex flex-col mb-4">
                 <p className="font-bold text-secondary-900 text-sm truncate">{user?.name || user?.email?.split('@')[0]}</p>
                 <p className="text-secondary-400 text-xs truncate font-medium">{user?.email}</p>
               </div>
             )}
